...
function ProjectCard() {
  ...
  return (
    <ul className='grid justify-center w-full grid-cols-1 gap-4 px-2 py-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 lg:gap-3 lg:justify-start lg:px-2'>
      {ProjectCardItems.slice(0).reverse().map((project, index) => ( 
        <li key={project.id} className='h-[auto] Container-Bg border border-white/15 rounded-xl'>
          <div className='bg-transparent Project-Card p-2 shadow-custom h-[28rem] rounded-xl flex flex-col gap-2'>
            ...
            <section className='flex flex-col gap-1 rounded-2xl'>
              ...
              <div className="w-full h-auto md:max-h-[5rem] gap-2 bg-gradient-to-r from-gray-50/20 to-black/50 rounded-xl flex flex-wrap overflow-y-auto p-2 no-scrollbar">
